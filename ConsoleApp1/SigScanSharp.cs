...
namespace BasicESP
{
    public class SigScanSharp
    {
        ...
        public bool SelectModule(ProcessModule targetModule)
        {
            ...
            return NativeMethods.ReadProcessMemory(g_hProcess, g_lpModuleBase, g_arrModuleBuffer, (UInt64)targetModule.ModuleMemorySize, out IntPtr bytesRead);
        }
        ...
        public ulong FindOffset(ulong instruction)
        {
            byte[] buffer = new byte[4];

            NativeMethods.ReadProcessMemory(g_hProcess, instruction + 3, buffer, 4, out IntPtr BytesRead);
            var instructionOffset = BitConverter.ToInt32(buffer, 0);

            return (ulong)((Int64)instruction + instructionOffset + 7);
        }
